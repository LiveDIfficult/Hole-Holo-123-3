...
module.exports = {
    config: {
        name: "softban",
        aliases: ["sb", "sremove", "slock"],
        usage: "<user> <reason>",
        category: "moderation",
        description: "Softban a member in the discord!",
...
message.delete()
banMember.send(`Hi, you got a softbanned on the server **${message.guild.name}** for: ${reason}`).then(() =>
message.guild.ban(banMember, { days: 1, reason: reason})).then(() => message.guild.unban(banMember.id, { reason: "Softban"})).then(m => m.delete(5000)).catch(err => console.log(err))

let embed = new Discord.RichEmbed()
.setColor("#fae034")
.setTitle("Success Softbanned")
.setThumbnail(message.guild.iconURL)
.addField("Type:", "**Softban**", true)
.addField("Author Command", message.author.username, true)
.addField("Softbanned User:", `${banMember.username} (${banMember.discriminator})`)
